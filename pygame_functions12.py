        ...
        for row, tiles in enumerate(map_data):
                for col, tile in enumerate(tiles):
                    if tile in base64dict:
                        ...
                        heartContainer.move(col*32, row*32)
                        self.Items.append(heartContainer)
                    if tile not in base64dict:
                        if tile == ",":
                            thisGround = Wall(self.images[34])
                        else:
...
class Item(newSprite):
    """
    This is a generic class for all items
    """
...
class Rupee(Item):
    """
    This sprite increases the players Rupees by 1
    """
    def __init__(self):
...
class Heart(Item):
    """
    This object regenerates 1 health
    """
    def __init__(self):
...
class HeartContainer(Item):
    """
    This object increases players maxHealth by 1
    """
    def __init__(self):
        Item.__init__(self,"Hearts.png", 3)
        self.maxHealth = 1
...
class Projectile(newSprite):
    """A generic base class for projectiles"""
...
class Player(newSprite):
    ...
    def __init__(self):
        ...
        #self.music = makeMusic("linkMusic.mp3")
        #playMusic()
        self.rect.x = 500
        self.rect.y = 350
        self.speed = 4
    ...
    def hit(self, walls):
        ...
        if self.rect.y <0:
            ...
            self.rect.y = 768
        for wall in walls:
            if touching(wall, self):
                if self.rect.y > 384:
                    self.rect.y = wall.rect.y-32
                else:
                    self.rect.y = wall.rect.y+32
...
class Gibdos(Enemy):
    def __init__(self):
        Enemy.__init__(self, "gibdos.png", 1, 2)
        self.health = 3
        self.speed = 4
        self.orientation = 0
    def move(self, frame):
...
def Make_Save(conn, game):
    sql = ''' INSERT INTO saveFiles(location, name, health, rupee) VALUES(?,?,?,?)'''
    cur = conn.cursor()
...
def Save(conn, game):
    sql = '''UPDATE saveFiles SET health = ?, rupee = ? WHERE location = ? '''
    cur = conn.cursor()
    cur.execute(sql, game)
    conn.commit()
    print("Saved")

def Delete_Game(conn, location):
    sql = 'DELETE FROM saveFiles WHERE location = ?'
    cur = conn.cursor()
    cur.execute(sql, (location,))
    conn.commit()
    sql = "SELECT * FROM saveFiles"
    cur.execute(sql)
    rows = cur.fetchall()
    newIndex = 1
    for row in rows:
        oldIndex = row[0]
        if row[0] != newIndex:
            sql = '''UPDATE saveFiles SET location = ? WHERE location = ?'''
            print(sql, (newIndex, oldIndex))
            cur.execute(sql, (newIndex, oldIndex))
        newIndex += 1

def Select(conn, fileNum):
    sql = "SELECT * FROM saveFiles WHERE location = ?"
    cur = conn.cursor()
    cur.execute(sql, (fileNum,))
    rows = cur.fetchall()
    lastRow = len(rows)
    if lastRow:
...
def MakeConn():
    db = "ZeldaSaveFile.db"
    sql_create_table = """ CREATE TABLE IF NOT EXISTS saveFiles (location integer PRIMARY KEY, name text, health integer, rupee integer);"""
...
def loadGame(conn, fileNum, link):
    if conn is not None:
        game = Select(conn, fileNum)
        if game:
            link.health = game[2]
            link.rupee = game[3]
        #game = (1, 'Joe', 10, 20)
        #saveNum = Make_Save(conn, game)
        return game
    else:
        print("Cannot use Database")
        return None
...
sword_shoot = makeSound("LOZ_Sword_Shoot.wav")
